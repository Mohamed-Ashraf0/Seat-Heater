...
/* Arrays to store timestamp for runtime measurements (extra index for idel task)*/
extern uint32_t ullTasksOutTime[11 + 1];
extern uint32_t ullTasksInTime[11 + 1];
extern uint32_t ullTasksTotalTime[11 + 1];

#define traceTASK_SWITCHED_IN()                                    \
do{                                                                \
    uint32_t taskInTag = (uint32_t)(pxCurrentTCB->pxTaskTag);          \
    ullTasksInTime[taskInTag] = GPTM_WTimer0Read();                \
}while(0);

#define traceTASK_SWITCHED_OUT()                                                                 \
do{                                                                                              \
    uint32_t taskOutTag = (uint32_t)(pxCurrentTCB->pxTaskTag);                                       \
