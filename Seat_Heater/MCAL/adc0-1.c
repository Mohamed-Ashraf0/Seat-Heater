...
void ADC0_Init(void)
{
    ...
    ADC0_EMUX_R =0xFFF0;//sampler zero trigered by processor (ADCPSSI)
    ADC0_SSMUX0_R=0;//analog input 0 enter rfrom mux0
    ADC0_SSCTL0_R|= (1<<2)|(1<<1);//set end bit , interupt flag
    ADC0_IM_R |=(1<<0);
    NVIC_PRI3_R = (NVIC_PRI3_R & 0xFF1FFFFF) | (6<<21);
    NVIC_EN0_R |=1<<14;
...
void ADC1_Init(void)
{
    ...
    ADC1_EMUX_R =0xFFF0;//sampler zero trigered by processor (ADCPSSI)
    ADC1_SSMUX0_R=1;//analog input 1 enter rfrom mux0
    ADC1_SSCTL0_R|= (1<<2)|(1<<1);//set end bit , interupt flag
    ADC1_IM_R |=(1<<0);
    NVIC_PRI12_R = (NVIC_PRI12_R & 0xFFFFFF1F) | (6<<5);
    NVIC_EN1_R |=1<<16;
...
{
    return ((ADC0_SSFIFO0_R*TARGET_MAX)/ADC_MAX);
}
uint32_t ADC1_Read(void)
{
    return ((ADC1_SSFIFO0_R*TARGET_MAX)/ADC_MAX);
}
